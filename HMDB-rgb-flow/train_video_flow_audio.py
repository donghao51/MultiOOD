...
def train_one_step(model, clip, labels, flow, model_flow, spectrogram, audio_cls_model, epoch_i):
    ...
    f_predict, flow_emd = model_flow.module.cls_head(flow_feat)

    predict = mlp_cls(v_emd, audio_emd, flow_emd)
    ...
    if args.use_a2d:
        ...
        for i in range(len(v_predict)):
            ...
            predicted_array_without_gt_f = torch.cat([f_predict[i, :label], f_predict[i, (label+1):]], dim=0)
            predicted_f_without_gt.append(predicted_array_without_gt_f.unsqueeze(0))
            predicted_array_without_gt_a = torch.cat([audio_predict[i, :label], audio_predict[i, (label+1):]], dim=0)
        ...
        predicted_f_without_gt = torch.cat(predicted_f_without_gt, dim=0)
        predicted_a_without_gt = torch.cat(predicted_a_without_gt, dim=0)

        if args.a2d_max_l1:
            a2d_loss1 = -nn.L1Loss()(nn.Softmax(dim=1)(predicted_v_without_gt), nn.Softmax(dim=1)(predicted_a_without_gt))
            a2d_loss2 = -nn.L1Loss()(nn.Softmax(dim=1)(predicted_f_without_gt), nn.Softmax(dim=1)(predicted_a_without_gt))
            a2d_loss3 = -nn.L1Loss()(nn.Softmax(dim=1)(predicted_v_without_gt), nn.Softmax(dim=1)(predicted_f_without_gt))
            a2d_loss = (a2d_loss1 + a2d_loss2 + a2d_loss3) / 3
        elif args.a2d_max_l2:
            a2d_loss1 = -nn.MSELoss()(nn.Softmax(dim=1)(predicted_v_without_gt), nn.Softmax(dim=1)(predicted_a_without_gt))
            a2d_loss2 = -nn.MSELoss()(nn.Softmax(dim=1)(predicted_f_without_gt), nn.Softmax(dim=1)(predicted_a_without_gt))
            a2d_loss3 = -nn.MSELoss()(nn.Softmax(dim=1)(predicted_v_without_gt), nn.Softmax(dim=1)(predicted_f_without_gt))
            a2d_loss = (a2d_loss1 + a2d_loss2 + a2d_loss3) / 3
        elif args.a2d_max_hellinger:
            a2d_loss1 = -hellinger_distance(F.softmax(predicted_v_without_gt, dim=1), F.softmax(predicted_a_without_gt, dim=1))
            a2d_loss2 = -hellinger_distance(F.softmax(predicted_f_without_gt, dim=1), F.softmax(predicted_a_without_gt, dim=1))
            a2d_loss3 = -hellinger_distance(F.softmax(predicted_v_without_gt, dim=1), F.softmax(predicted_f_without_gt, dim=1))
            a2d_loss = (a2d_loss1 + a2d_loss2 + a2d_loss3) / 3
            ...
            a2d_loss3 = -wasserstein_distance(F.softmax(predicted_f_without_gt, dim=1), F.softmax(predicted_a_without_gt, dim=1))
            a2d_loss4 = -wasserstein_distance(F.softmax(predicted_a_without_gt, dim=1), F.softmax(predicted_f_without_gt, dim=1))
            a2d_loss5 = -wasserstein_distance(F.softmax(predicted_v_without_gt, dim=1), F.softmax(predicted_f_without_gt, dim=1))
            a2d_loss6 = -wasserstein_distance(F.softmax(predicted_f_without_gt, dim=1), F.softmax(predicted_v_without_gt, dim=1))
            a2d_loss = (a2d_loss1 + a2d_loss2 + a2d_loss3 + a2d_loss4 + a2d_loss5 + a2d_loss6) / 6
        
        loss = loss + args.a2d_ratio * a2d_loss

    if args.use_npmix:
        output = torch.cat((v_emd, audio_emd, flow_emd), dim=1)
        sum_temp = 0
        ...
        if (sum_temp == num_class * args.sample_number) and (epoch_i < args.start_epoch):
            ...
            if len(ood_samples) != 0:
                ...
                a_pred_ood = audio_cls_model.fc(ood_samples[:,v_dim:v_dim+a_dim])
                f_pred_ood = model_flow.module.cls_head.fc_cls(ood_samples[:,v_dim+a_dim:])

                if args.max_ood_l1:
                    a2d_loss_ood1 = -nn.L1Loss()(nn.Softmax(dim=1)(v_pred_ood), nn.Softmax(dim=1)(a_pred_ood))
                    a2d_loss_ood2 = -nn.L1Loss()(nn.Softmax(dim=1)(f_pred_ood), nn.Softmax(dim=1)(a_pred_ood))
                    a2d_loss_ood3 = -nn.L1Loss()(nn.Softmax(dim=1)(v_pred_ood), nn.Softmax(dim=1)(f_pred_ood))
                    a2d_loss_ood = (a2d_loss_ood1 + a2d_loss_ood2 + a2d_loss_ood3) / 3
                elif args.max_ood_l2:
                    a2d_loss_ood1 = -nn.MSELoss()(nn.Softmax(dim=1)(v_pred_ood), nn.Softmax(dim=1)(a_pred_ood))
                    a2d_loss_ood2 = -nn.MSELoss()(nn.Softmax(dim=1)(f_pred_ood), nn.Softmax(dim=1)(a_pred_ood))
                    a2d_loss_ood3 = -nn.MSELoss()(nn.Softmax(dim=1)(v_pred_ood), nn.Softmax(dim=1)(f_pred_ood))
                    a2d_loss_ood = (a2d_loss_ood1 + a2d_loss_ood2 + a2d_loss_ood3) / 3
                elif args.max_ood_hellinger:
                    a2d_loss_ood1 = -hellinger_distance(F.softmax(v_pred_ood, dim=1), F.softmax(a_pred_ood, dim=1))
                    a2d_loss_ood2 = -hellinger_distance(F.softmax(f_pred_ood, dim=1), F.softmax(a_pred_ood, dim=1))
                    a2d_loss_ood3 = -hellinger_distance(F.softmax(v_pred_ood, dim=1), F.softmax(f_pred_ood, dim=1))
                    a2d_loss_ood = (a2d_loss_ood1 + a2d_loss_ood2 + a2d_loss_ood3) / 3
                    ...
                    a2d_loss_ood3 = -wasserstein_distance(F.softmax(f_pred_ood, dim=1), F.softmax(a_pred_ood, dim=1))
                    a2d_loss_ood4 = -wasserstein_distance(F.softmax(a_pred_ood, dim=1), F.softmax(f_pred_ood, dim=1))
                    a2d_loss_ood5 = -wasserstein_distance(F.softmax(v_pred_ood, dim=1), F.softmax(f_pred_ood, dim=1))
                    a2d_loss_ood6 = -wasserstein_distance(F.softmax(f_pred_ood, dim=1), F.softmax(v_pred_ood, dim=1))
                    a2d_loss_ood = (a2d_loss_ood1 + a2d_loss_ood2 + a2d_loss_ood3 + a2d_loss_ood4 + a2d_loss_ood5 + a2d_loss_ood6) / 6

                # max_ood_entropy
                v_pred_ood_ent = normalized_prediction_entropy(v_pred_ood)
                a_pred_ood_ent = normalized_prediction_entropy(a_pred_ood)
                f_pred_ood_ent = normalized_prediction_entropy(f_pred_ood)
                ood_entropy_loss = -(torch.mean(v_pred_ood_ent) + torch.mean(a_pred_ood_ent) + torch.mean(f_pred_ood_ent)) / 3
...
if __name__ == '__main__':
    ...
    if args.near_ood:
        log_name = "log_video_flow_audio_%s_near_ood_lr_%s_bsz_%s_%s_%s"%(str(args.dataset), str(args.lr), str(args.bsz), str(args.nepochs), args.opt)
    else:
        log_name = "log_video_flow_audio_%s_far_ood_lr_%s_bsz_%s_%s_%s"%(str(args.dataset), str(args.lr), str(args.bsz), str(args.nepochs), args.opt)
        ...
        for epoch_i in range(starting_epoch, args.nepochs):
            ...
            for split in splits:
                ...
                with tqdm.tqdm(total=len(dataloaders[split])) as pbar:
                    ...
                    if split == 'val':
                        ...
                        if currentvalAcc >= BestAcc:
                            ...
                            if args.save_best:
                                ...
                                if not args.resumef:
                                    prefix = log_name + '_best_'
                                    for filename in os.listdir(base_path_model):
                                        # Check if the file starts with the specified prefix
                                        if filename.startswith(prefix):
                                            # Construct the full path to the file
                                            file_path = os.path.join(base_path_model, filename)
                                            try:
                    ...
                    if args.save_checkpoint and epoch_i % 5 == 0:
                        save = {
                            'epoch': epoch_i,
                            'BestLoss': BestLoss,
                            'BestEpoch': BestEpoch,
                        ...
                        save['mlp_cls_state_dict'] = mlp_cls.state_dict()
                        torch.save(save, base_path_model + log_name + '_epoch_%s.pt'%(str(epoch_i)))
